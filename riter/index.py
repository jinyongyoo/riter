...
class JointEmbeddingIndex(BaseIndex):
    ...
    Args:
        ...
        img_preprocessor (torchvision.Transforms): The transformation steps applied to PIL.Image.Image objects to turn them into tensors.
        ...
        tokenizer (obj): Tokenizer that when called (via `__call__`), returns list of ids that could be converted into a tensor and passed
            to the `text_encoder`.
    """
    def __init__(self, dim, img_encoder, img_preprocessor, text_encoder, tokenizer):
        self.dim = dim
    ...
    def query(self, query, top_k=20, index_type="both", reduction="mean"):
        """
        ...
        if isinstance(query, PIL.Image.Image):
            image = self.img_preprocessor(query)
            with torch.no_grad():
            ...
            with torch.no_grad():
                query_vec = self.text_encoder(text_tokens, [length]).detach().cpu().numpy()
        else:
            raise ValueError(
        ...
        if index_type == "both":
            ...
            indices = sorted(combined_scores, key=lambda k: combined_scores[k], reverse=True)[:top_k]
